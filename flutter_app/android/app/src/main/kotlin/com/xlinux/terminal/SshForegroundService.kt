 ...
 * Features: auto-restart, keepalive, system DNS resolution.
 */
class SshForegroundService : Service() {
    ...
    private fun startSshd(port: Int) {
        ...
        Thread {
            try {
                ...
                val cmd = pm.buildGatewayCommand(
                    "echo 'root:xlinux' | chpasswd 2>/dev/null; " +
                    "sed -i 's/#ListenAddress.*/ListenAddress 0.0.0.0/' /etc/ssh/sshd_config 2>/dev/null || " +
                    "sed -i 's/ListenAddress.*/ListenAddress 0.0.0.0/' /etc/ssh/sshd_config 2>/dev/null || " +
                    "echo 'ListenAddress 0.0.0.0' >> /etc/ssh/sshd_config 2>/dev/null; " +
                ...
                while (restartCount <= maxRestarts && isRunning) {
                    ...
                    updateNotification(when {
                        restartCount == 0 -> "SSH running on port $port"
                        else -> "SSH restarted (attempt $restartCount)"
                    })
                    ...
                    Thread {
                        try {
                            ...
                            while (stderrReader.readLine().also { line = it } != null) {
                                android.util.Log.w("XLinux-SSH", line ?: "")
                            }
                        } catch (_: Exception) {}
                    }.start()
                    ...
                    if (restartCount <= maxRestarts) {
                        val delay = when {
                            restartCount <= 3 -> 2000L
                            restartCount <= 10 -> 3000L
                            else -> 5000L
                        }
                        updateNotification("SSH exited ($exitCode), restarting in ${delay/1000}s...")
                        Thread.sleep(delay)
                    } else {
                        isRunning = false
    ...
    private fun acquireWakeLock() {
        ...
        wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "XLinux::SSH").apply {
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(CHANNEL_ID, "XLinux SSH", NotificationManager.IMPORTANCE_LOW).apply {
                description = "SSH server running in background"
            }
